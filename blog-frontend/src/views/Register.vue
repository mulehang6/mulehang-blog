      ...
      <Card class="shadow-xl">
        ...
        <CardContent>
          <form @submit.prevent="handleRegister" class="space-y-4">
            ...
            <!-- 成功提示 -->
            <div v-if="successMessage" class="bg-green-500/10 border border-green-500/20 text-green-600 text-sm rounded-md p-3">
              {{ successMessage }}
            </div>
...
async function handleRegister() {
  ...
  try {
    // 调用注册 API
    await userStore.register(formData.username, formData.password, formData.email, formData.nickname)
