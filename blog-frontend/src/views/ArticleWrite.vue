  ...
  <div class="min-h-screen bg-background">
    <div class="container mx-auto px-4 py-8">
      <div class="max-w-4xl mx-auto">
        <!-- 页面标题 -->
        <div class="mb-8">
          <h1 class="text-3xl font-bold">{{ isEditMode ? '编辑文章' : '写文章' }}</h1>
          <p class="text-muted-foreground mt-2">{{ isEditMode ? '修改文章内容' : '创作你的新文章' }}</p>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <div v-if="form.coverUrl" class="mt-2">
              <img :src="form.coverUrl" alt="封面预览" class="max-w-xs rounded-md border" />
            </div>
          </div>
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-2">
                <div class="relative">
                  ...
                  >
                    <!-- 已选分类 -->
                    <div v-if="form.categoryId" class="p-2 border-b bg-muted/50">
                      <div class="flex items-center justify-between text-sm">
                        <span class="font-medium">已选：{{ getCategoryName(form.categoryId) }}</span>
                        <button
                          type="button"
                          @click="clearCategory"
                    ...
                    <!-- 创建新分类 -->
                    <button
                      v-if="categorySearchKeyword && !filteredCategories.some(c => c.name === categorySearchKeyword)"
                      type="button"
                    ...
                    <button
                      ...
                      :class="{ 'bg-primary/10': form.categoryId === category.id }"
                    >
                      <span>{{ category.name }}</span>
                      <!-- 删除按钮（仅创建者可见） -->
                      <button
                        v-if="category.creatorId && userStore.userInfo?.id === category.creatorId"
                        type="button"
                    ...
                    <div v-if="filteredCategories.length === 0 && !categorySearchKeyword" class="px-3 py-2 text-sm text-muted-foreground">
                      暂无分类
                    </div>
                  </div>
                </div>
                <!-- 已选分类显示 -->
                <div v-if="form.categoryId && !showCategoryDropdown" class="text-sm text-muted-foreground">
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-2">
                ...
                <div class="flex flex-wrap gap-2 p-2 border rounded-md min-h-10">
                  ...
                  <span v-if="form.tagIds.length === 0" class="text-sm text-muted-foreground">点击下方添加标签</span>
                </div>
                <!-- 标签搜索 -->
                <div class="relative">
                  <input
                  ...
                  >
                    <!-- 创建新标签 -->
                    <button
                      v-if="tagSearchKeyword && !filteredTags.some(t => t.name === tagSearchKeyword)"
                      type="button"
                      @click="createNewTag"
                      class="w-full text-left px-3 py-2 hover:bg-muted transition-colors text-sm flex items-center gap-2 text-primary"
                    >
                    ...
                    >
                      <span>
                        <span>{{ tag.name }}</span>
                        <span v-if="form.tagIds.includes(tag.id)" class="ml-2 text-primary">✓</span>
                      </span>
                      <!-- 删除按钮（仅创建者可见） -->
                      <button
                        v-if="tag.creatorId && userStore.userInfo?.id === tag.creatorId"
                        type="button"
                    ...
                    <div v-if="filteredTags.length === 0 && !tagSearchKeyword" class="px-3 py-2 text-sm text-muted-foreground">
                      暂无标签
                    </div>
                  </div>
                </div>
          ...
          <!-- Markdown 编辑器 -->
          <div class="space-y-2">
            <label class="text-sm font-medium">文章内容 (Markdown) <span class="text-destructive">*</span></label>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div class="space-y-2">
                ...
                <div class="border rounded-md p-4 min-h-125 overflow-auto bg-card">
                  <MarkdownRenderer v-if="form.contentMd" :content="form.contentMd" />
          ...
          <div class="flex gap-4 justify-end pt-6 border-t">
            ...
            <Button type="button" variant="secondary" @click="handleSaveDraft" :disabled="loading">
              保存草稿
            </Button>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            确定要删除{{ deleteDialog.type === 'category' ? '分类' : '标签' }}「{{ deleteDialog.name }}」吗？<br />
            此操作不可恢复。
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
async function createNewCategory() {
  ...
  try {
    ...
    categories.value.push({ id: categoryId, name, slug, articleCount: 0, creatorId: currentUserId })
...
async function createNewTag() {
  ...
  try {
    ...
    // 添加到列表（包含 creatorId）
    const currentUserId = userStore.userInfo?.id
    tags.value.push({ id: tagId, name, slug, articleCount: 0, creatorId: currentUserId })
...
async function confirmDelete() {
  if (deleteDialog.value.type === 'category') {
    await deleteCategoryConfirmed(deleteDialog.value.id, deleteDialog.value.name)
  } else {
 ...
 * 删除分类（已确认）
 */
async function deleteCategoryConfirmed(categoryId: number, categoryName: string) {
  try {
    ...
    toast.error('删除失败', {
      description: error.response?.data?.msg || error.response?.data?.message || error.message || '请重试'
    })
  }
}
